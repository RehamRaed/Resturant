...
export default function Order() {
  ...
  return (
    <section className="py-20 text-center px-6 md:px-24 flex flex-col text-left">
      <div>
        <h2 className="text-4xl md:text-5xl font-semibold mb-4 transition-all duration-500 ease-in-out">
          ORDER ONLINE
        </h2>
        <p
          className={`${merienda.className} text-md text-gray-700 transition-all duration-500 ease-in-out`}
        >
        ...
        <div className="flex my-5 gap-2 px-4 py-1 rounded-full w-fit border border-black items-center">
          ...
          <h2 className="text-gray-800 font-semibold text-sm">Accepting Orders</h2>
        </div>
        ...
        <div className="flex w-[90%] [@media(min-width:649px)]:w-[80%]">
          {["pickup", "delivery"].map((type) => (
            <button
              ...
              className={`w-50 text-sm py-3 font-semibold transition-all duration-300 ease-in-out
                ${
                  method === type
        ...
        {method === "pickup" ? (
          <div className="flex flex-col mt-2 gap-3 md:gap-6 md:flex-row items-start md:items-center w-full transition-all duration-500 ease-in-out">
            <h4 className="flex items-center gap-1 w-fit">
              ...
              <button
                onClick={() => setModalOpen(true)}
                className="text-[15px] text-gray-600 underline cursor-pointer ml-1"
              >
                Change
              </button>
            </h4>
          ...
          <div className="mt-2 text-left transition-all duration-500 ease-in-out">
            <h4 className="flex items-center gap-1">
              <Image src="/site.png" alt="" width={17} height={17} />
